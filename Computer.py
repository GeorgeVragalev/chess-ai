...
def minimax(board, depth, alpha, beta, max_player, save_move, data):
    ...
    if max_player:
        return max_value(board, depth, alpha, beta, save_move, data)
    else:
        return min_value(board, depth, alpha, beta, save_move, data)


def max_value(board, depth, alpha, beta, save_move, data):
    max_eval = -math.inf
    for move in board.generate_moves(max_player=True):  # New move generator function
        piece = move[0]
        final_pos = move[1]
        board.make_move(piece, final_pos[0], final_pos[1], keep_history=True)
        evaluation = minimax(board, depth - 1, alpha, beta, False, save_move, data)[1]
        board.unmake_move(piece)
        max_eval = max(max_eval, evaluation)
        alpha = max(alpha, evaluation)
        if beta <= alpha:
    ...
    transposition_table[board.compute_hash()] = (move, max_eval)
    return move, max_eval


def min_value(board, depth, alpha, beta, save_move, data):
    min_eval = math.inf
    for move in board.generate_moves(max_player=False):  # New move generator function
        piece = move[0]
        final_pos = move[1]
        board.make_move(piece, final_pos[0], final_pos[1], keep_history=True)
        evaluation = minimax(board, depth - 1, alpha, beta, True, save_move, data)[1]
        board.unmake_move(piece)
        min_eval = min(min_eval, evaluation)
        beta = min(beta, evaluation)
        if beta <= alpha:
            break
    # Store result in transposition table
    transposition_table[board.hash()] = (move, min_eval)
...
def progressive_deepening(board, max_depth):
    ...
    for depth in range(1, max_depth + 1):
        current_evaluation = minimax(board, depth, -math.inf, math.inf, True, False, [[], 0])[1]
        if current_evaluation > best_evaluation:
            best_evaluation = current_evaluation
            best_move_data = minimax(board, depth, -math.inf, math.inf, True, False, [[], 0])
